...
/*************************************************************************************************/
 ...
 /***************************************888滑动区************************************************/
    ...
    connect(this,SIGNAL(createChatSocket()),&man,SLOT(SetUpChatSocker()));
     ...
     ReceiveFriendTcp=new ReceiveFriendTcpMessage(tcpSocket_FriendList);
     connect(ReceiveFriendTcp,SIGNAL(SendTheList(QList<Friend>)),this,SLOT(SetFriendWidget(QList<Friend>)));
}
void PageMe::SetFriendWidget(QList<Friend> list_)
{
    FriendList=list_;
