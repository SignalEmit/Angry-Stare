...
void MyThread::OpenMessageDatabase()
{
...
void MyThread::receive_information()
{
    if(!is_receivePicture)
    {
        ...
        switch (Judge.toInt()) {
            ...
            case 22:SelectMyPaySuccessGoods();break;        //查找并发送支付成功的商品
            case 23:SendMyMessage();break;                      //发送用户自己的账号信息
            case 24:ChangeMyMessage();break;                //修改自己的信息
        default:
            break;
        }
    }
...
void MyThread::SendTcpPaySuccessGoods(PayMessage &message,QTcpSocket *tcpSocket)
{
...
void MyThread::SelectMyPaySuccessGoods()
{
    QString useraccount;
    receiveTcpMessage(useraccount,receiveSocket);
    db.open();
    QSqlQuery SelectPaySuccessGoodsCount;
    SelectPaySuccessGoodsCount.exec(tr("select count(*) from PaySuccessfullyProducts where PayUserAccount='%0'").arg(useraccount));
    SelectPaySuccessGoodsCount.next();
    SendTcpMessage(SelectPaySuccessGoodsCount.value(0).toString(),receiveSocket);
    QSqlQuery SelectPaySuccessGoods;
    SelectPaySuccessGoods.exec(tr("select PaySuccessfullyProducts.OrderCommodityCode,PaySuccessfullyProducts.PayGoodsMassUnit,PaySuccessfullyProducts.PayGoodsMoney,PaySuccessfullyProducts.PayGoodsNumber,FarmProductsMessage.CommodityCode,FarmProductsMessage.ContactNumber,FarmProductsMessage.Cost,FarmProductsMessage.MassUnit,FarmProductsMessage.Number,FarmProductsMessage.PictureLocation,FarmProductsMessage.Postage,FarmProductsMessage.ProductName,FarmProductsMessage.Promulgator,FarmProductsMessage.Synopsis from PaySuccessfullyProducts,FarmProductsMessage where PayUserAccount='%0' and PaySuccessfullyProducts.PayCommodityCode=FarmProductsMessage.CommodityCode").arg(useraccount));
    while(SelectPaySuccessGoods.next())
    {

        PayMessage theGoodsPayMessage(SelectPaySuccessGoods.value("OrderCommodityCode").toString(),
                                      SelectPaySuccessGoods.value("PayGoodsMassUnit").toString(),
                                      SelectPaySuccessGoods.value("PayGoodsNumber").toInt(),
                                      SelectPaySuccessGoods.value("PayGoodsMoney").toFloat(),
                                      SelectPaySuccessGoods.value("CommodityCode").toString(),
                                      SelectPaySuccessGoods.value("ProductName").toString(),
                                      SelectPaySuccessGoods.value("Cost").toFloat(),
                                      SelectPaySuccessGoods.value("Number").toFloat(),
                                      SelectPaySuccessGoods.value("MassUnit").toString(),
                                      SelectPaySuccessGoods.value("Synopsis").toString(),
                                      SelectPaySuccessGoods.value("PictureLocation").toString(),
                                      SelectPaySuccessGoods.value("Postage").toFloat(),
                                      SelectPaySuccessGoods.value("Promulgator").toString(),
                                      SelectPaySuccessGoods.value("ContactNumber").toString());
        SendTcpPaySuccessGoods(theGoodsPayMessage,receiveSocket);
    }
    db.close();
...
void MyThread::SendMyMessage()
{
    QString userAccount;
    receiveTcpMessage(userAccount,receiveSocket);
    QSqlQuery selectMyMessage;                      //查询自己的账号信息
    selectMyMessage.exec(tr("select Name,Age,Nickname,Sex,Area from UserMessage where Account='%0'").arg(userAccount));
    if(selectMyMessage.next())
    {
        Friend MyMessage(selectMyMessage.value("Name").toString(),"",selectMyMessage.value("Age").toString(),selectMyMessage.value("Nickname").toString(),selectMyMessage.value("Sex").toString(),selectMyMessage.value("Area").toString(),true);
        QByteArray data;			//暂时要发送的数据
        QDataStream out(&data, QIODevice::WriteOnly);
        out.setVersion(QDataStream::Qt_5_8);						//设置数据流版本
        out << (quint16)0;
...
void MyThread::ReceiveMyMessage(Friend &myMessage,QTcpSocket *tcpSocket)
{
    QDataStream in(tcpSocket);
    in.setVersion(QDataStream::Qt_5_8);
    if (blockSize == 0)
...
void MyThread::ChangeMyMessage()
{
    QString waitChangAccount;      //待修改的账号
    receiveTcpMessage(waitChangAccount,receiveSocket);
    Friend myMessage;
    ReceiveMyMessage(myMessage,receiveSocket);
    QSqlQuery change(tr("update UserMessage set Name='%0',Age=%1,Nickname='%2',Sex='%3',Area='%4' where Account='%5'").arg(myMessage.FriendCount).arg(myMessage.Age.toInt()).arg(myMessage.NickName).arg(myMessage.Sex).arg(myMessage.Area).arg(waitChangAccount));
    if(change.exec())
        SendTcpMessage("修改成功",receiveSocket);
    receiveSocket->disconnectFromHost();
}
