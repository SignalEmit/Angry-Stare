...
{
    ui->setupUi(this);
    is_click_Backbt=false;                          //点击了Backbt
    ...
    connect(Offline, SIGNAL(connected()), this, SLOT(sendMessage()));
     ...
     tcpSocket_receiveMyMessage=new QTcpSocket(this);
     connect(tcpSocket_receiveMyMessage, SIGNAL(connected()), this, SLOT(sendMessage()));
     connect(tcpSocket_receiveMyMessage, SIGNAL(readyRead()), this, SLOT(receiveMessage()));
 /************************网络********************************/
 /************************网络********************************/
...
//*****************设置搜索******************************//
    ...
    ui->widget_3->setFixedSize(width_Change(0.9930),newsreel_Button*2+50+heigth_Change(0.2)+75);               //设置滑动区大小
...
/**********************************************8创建指针对象*******************************************/
   ...
   /*****************************************888接收完推荐链表后显示界面然后接受图片********************/
    ...
    connect(this,SIGNAL(ChangeSettingPage(int,Friend)),&setting,SLOT(SetPage(int,Friend)));
    connect(&setting,SIGNAL(SetHeadPicture(QString)),this,SLOT(SetHeadPictureIcon(QString)));
    connect(&setting,SIGNAL(MyMessageChange(Friend)),this,SLOT(ChangeMymessage(Friend)));


    RecommendProcuct=new ReceiveProductTcpMessage(tcpSocket_Recommend);
    connect(RecommendProcuct,SIGNAL(SendTheList(QList<Procuct>)),this,SLOT(SetRecommendWidget(QList<Procuct>)));
}
void Interface::ChangeMymessage(Friend mymessage)
{
    MyMessage.Age=mymessage.Age;
    MyMessage.Area=mymessage.Area;
    MyMessage.FriendCount=mymessage.FriendCount;
    MyMessage.NickName=mymessage.NickName;
    MyMessage.Sex=mymessage.Sex;
}
...
void Interface::showEvent(QShowEvent *event)
{
   ...
   newConnect(Online);
   newConnect(tcpSocket_receiveMyMessage);
...
void Interface::SetRecommendWidget(QList<Procuct> list_)
{
    RecommendProcuctList=list_;
    for(int i=0;i<RecommendProcuctList.size();i++)
    {
        ui->widget_3->setFixedSize(width_Change(0.9930),newsreel_Button*2+heigth_Change(0.16)*(i+1)+100+heigth_Change(0.2)+25);
...
void Interface::receiveMessage()
{
    ...
    else if(sender()==tcpSocket_receiveMyMessage)
    {
        ReceiveFriendTcpMessage recevieMyMessage(tcpSocket_receiveMyMessage);
        recevieMyMessage.ReceiveOneFriendMessage(MyMessage);
    }
}
...
void Interface::sendMessage()
{
    ...
    {
        SendTcpMessage("23",tcpSocket_receiveMyMessage);
        SendTcpMessage(UserAccount,tcpSocket_receiveMyMessage);
    }
}
...
{
    emit ChangeSettingPage(0,MyMessage);
    setting.show();
}
...
{
    emit ChangeSettingPage(1,MyMessage);
    setting.show();
}


void Interface::on_AllOrdersBt_clicked()
{
    orderforgoods.show();
}

void Interface::on_BackBt_clicked()
{
    Online->disconnectFromHost();               //断开与服务器的连接  就是间接的做了下线的操作  否则未清空的指针内部存储着一个online重新登录就会gg
    is_click_Backbt=true;
    this->close();
    emit loginWidgetShow();
}
