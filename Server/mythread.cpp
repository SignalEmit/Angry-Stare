...
extern QList<ChatUser> ChatUserList;                   //聊天界面套接字
extern QList<OnlinePeople> OnlinePeopleList;           //在线用户list
MyThread::MyThread()
{
...
void MyThread::receive_information()
{
    ...
    if(!is_receivePicture)
    {
        ...
        switch (Judge.toInt()) {
            ...
            case 14:ReceiveManyPicture();break;            //接收来自客户端的各种各样的图片
...
void MyThread::SendSearchMessage()
{
    ...
    if(AddCount.next())
    {
        ...
        {
            Procuct newProduct(CheckProduct.value(0).toString(),CheckProduct.value(1).toString(),CheckProduct.value(2).toFloat(),CheckProduct.value(3).toInt(),CheckProduct.value(4).toString(),CheckProduct.value(5).toString(),CheckProduct.value(6).toString(),CheckProduct.value(7).toFloat(),CheckProduct.value(8).toString(),CheckProduct.value(9).toString());
                    //新产品
...
void MyThread::ReceiveSendChatMessage()              //接收并发送信息
{
    ...
    {
        if(ChatUserList.at(i).UserAccount==FriendAccount)
        {
            JudgeStockClose="发送朋友消息";
            connect(ChatUserList.at(i).tcpSocketChatUser, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(displayError(QAbstractSocket::SocketError)));
            SendTcpMessage(SendUserAccount,ChatUserList.at(i).tcpSocketChatUser);
            SendTcpMessage(chatMessage,ChatUserList.at(i).tcpSocketChatUser);
            break;
        }
        if(i==ChatUserList.size()-1)
        {    for(int i=0;i<OnlinePeopleList.size();i++)
            {
                if(OnlinePeopleList.at(i).UserAccount==FriendAccount)
                {
                    SendTcpMessage("主界面接收消息",OnlinePeopleList.at(i).tcpSocketOnline);
                    SendTcpMessage(SendUserAccount,OnlinePeopleList.at(i).tcpSocketOnline);
                    SendTcpMessage(chatMessage,OnlinePeopleList.at(i).tcpSocketOnline);
                    qDebug()<<"主界面接收消息然后阿桑大事大撒大撒自行车选择12330000............";
                    break;
                }
                else
                {
                    qDebug()<<"在服务器上存文件等账号上线提示";
                }
            }
         }
    }
...
void MyThread::receiveChatSocket()
{
    QString UserAccount;
    receiveTcpMessage(UserAccount,receiveSocket);
    ChatUser oneUser(receiveSocket,UserAccount);


   for(int i=0;i<ChatUserList.size();i++)          //便利链表防止套接字重复
      if(UserAccount==ChatUserList.at(i).UserAccount)
       {
          ChatUserList.removeAt(i);               //删除重复
          break;
       }
    ChatUserList.push_back(oneUser);              //添加
...
void MyThread::OffLine()
{
    ...
    receiveTcpMessage(UserAccount,receiveSocket);
    for(int i=0;i<OnlinePeopleList.size();i++)
        if(OnlinePeopleList.at(i).UserAccount==UserAccount)
        {
            emit OffLinePeopleNumber(i);
            qDebug()<<"下线"<<i;
            SendTcpMessage("下线吧",receiveSocket);
            for(int i=0;i<ChatUserList.size();i++)
                if(ChatUserList.at(i).UserAccount==UserAccount)
                 {
...
void MyThread::SaveOnlinePeople()
{
    ...
    for(int i=0;i<OnlinePeopleList.size();i++)
        if(OnlinePeopleList.at(i).UserAccount==UserAccount)
        {
            SendTcpMessage("异地登录",OnlinePeopleList.at(i).tcpSocketOnline);
            emit OffLinePeopleNumber(i);
            break;
        }
...
{
    out<<data.FriendCount<<data.Remark<<data.Age<<data.NickName<<data.Sex<<data.Area<<data.NoYesFriend;
    return out;
}
/******************************发送朋友列表*******************************/
