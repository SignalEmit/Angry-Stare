...
void Widget::receiveMessage()
{
    ...
    if(sender()==tcpSocket_Login)                               //检测登录
    {
        ...
        if(Judge=="密码账号正确")
        {
            ...
            UserAccount=ui->Account_lineedit->currentText();           //给纪录的账号赋值账号赋值
    ...
    else if(sender()==tcpSocket_CheckAccountRegisterLogIn)      //检测账号是否存在
    {
        ...
        tcpSocket_CheckAccountRegisterLogIn->disconnectFromHost();
    }
    else if(sender()==tcpSocket_RegisterLogIn)      //检测账号是否存在
    {
        ...
        tcpSocket_RegisterLogIn->disconnectFromHost();
    }
    else if(sender()==tcpSocket_ForgetPassword)//检测忘记密码
    {
        ...
        tcpSocket_ForgetPassword->disconnectFromHost();
    }
}
Widget::~Widget()
{
