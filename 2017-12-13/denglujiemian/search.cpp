...
/******************************8商品查询显示界面********************************/
    ...
    QScroller::grabGesture(ui->SearchListWidget, QScroller::TouchGesture);
    ...
    ReceiveProcuctMessageTcp=new ReceiveProductTcpMessage(ReceiveProductMessage);
    connect(ReceiveProcuctMessageTcp,SIGNAL(SendTheList(QList<Procuct>)),this,SLOT(SetProductWidget(QList<Procuct>)));
    ReveiveSearchMessage=new ReceiveSearchQStringTcpMessage(SearchMessage);
    connect(ReveiveSearchMessage,SIGNAL(SendTheList(QList<QString>)),this,SLOT(SetSearchMessage(QList<QString>)));
}
void Search::SetProductWidget(QList<Procuct> list_)
{
    ProductList=list_;
    UpdataSearchProductView();
}

void Search::SetSearchMessage(QList<QString> list_)
{
    for(int i=0;i<list_.size();i++)
         ui->SearchListWidget->addItem(list_[i]);
